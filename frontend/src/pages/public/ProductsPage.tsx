...
export function ProductsPage() {
  ...
  // EXTRACT CATEGORIES FROM PRODUCTS (since backend filtering might not work)
  ...
  // FILTER PRODUCTS CLIENT-SIDE (since backend might not support it)
  ...
  // Load ALL products initially (no category filter on backend)
  useEffect(() => {
    fetchProducts({ 
      page: currentPage, 
      size: 50 // Increase size to get more products for client-side filtering
    });
  ...
  const handleCartAction = useCallback((success: boolean, message: string) => {
    showToast(message, success ? 'success' : 'error');
  }, [showToast]);

  // Memoize pagination buttons (disabled for now due to client-side filtering)
