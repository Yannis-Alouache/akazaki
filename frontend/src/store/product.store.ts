...
for (const product of response.products) {
  if (ApiValidators.validateProductResponse(product)) {
    ...
    errorLogger.logValidationError('Invalid product data received from API');
  }
}
    
    set({
      products: validatedProducts,
      currentPage: SafeTypeAssertions.safeToNumber(response.pageNumber, 0),
      totalPages: SafeTypeAssertions.safeToNumber(response.totalPages, 0),
      totalElements: SafeTypeAssertions.safeToNumber(response.totalElements, 0),
      isLoading: false,
    });
  } catch (error) {
