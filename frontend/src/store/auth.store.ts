...
import type { AuthState, LoginRequest, RegisterUserRequest, UserResponse } from '../types';
import { authService } from '../services';
import { FormValidators, ApiValidators, SafeTypeAssertions } from '../utils/validation';
    ...
    (set, get) => ({
      ...
      login: async (credentials) => {
        ...
        try {
          ...
          const userData = jwtUtils.extractUserData(payload, credentials.email);
          ...
          console.log('Login successful', { email: userData.email, admin: userData.admin });
          
        } catch (error: any) {
          const errorMessage = error.response?.data?.reason || error.message || 'Login failed';
          set({ 
            isAuthenticated: false, 
            token: null, 
            user: null, 
...
register: async (data) => {
  ...
  try {
    ...
    const errorMessage = error.response?.data?.reason || error.message || 'Registration failed';
    errorLogger.logValidationError(`Registration failed: ${errorMessage}`);
    set({ 
      isLoading: false,
      error: errorMessage 
    });
