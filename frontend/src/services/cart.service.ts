...
import { ApiValidators, SafeTypeAssertions, TypeValidator } from '../utils/validation';
...
export const cartService = {
  ...
  if (!SafeTypeAssertions.safeToNumber(productId) || productId <= 0) {
    errorLogger.logValidationError('Invalid product ID for quantity update');
    throw new Error('Invalid product ID');
  }
  
  // FIXED: Allow 0 quantity (for removal) - only reject negative numbers
  ...
  await apiClient.put(`/cart/items/${productId}?quantity=${data.quantity}`);
},
};