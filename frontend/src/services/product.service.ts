...
export const productService = {
  async getProducts(params?: ProductListRequest): Promise<ProductListResponse> {
  const response = await apiClient.get<ProductListResponse>('/products', { params });
  
  // Validate response structure
  if (!response.data || !Array.isArray(response.data.products)) {
    errorLogger.logValidationError('Invalid products list response structure');
  ...
  if (!ApiValidators.validateProductResponse(response.data)) {
    errorLogger.logValidationError(`Invalid product response for ID: ${id}`);
  ...
  async createProduct(data: CreateProductRequest): Promise<ProductResponse> {
    ...
    const response = await apiClient.post<ProductResponse>('/admin/products', formData, {
      headers: {
