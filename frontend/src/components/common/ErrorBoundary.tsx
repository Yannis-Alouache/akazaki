...
export type ErrorType = 'auth' | 'network' | 'validation' | 'runtime' | 'unknown';
...
class ErrorLogger {
  ...
  logValidationError(message: string, field?: string): void {
    this.log({
      type: 'validation',
      message: `Validation failed: ${message}${field ? ` (field: ${field})` : ''}`,
      timestamp: new Date(),
      url: window.location.href
    });
  }
...
// Error boundary props - FIXED: Added missing interface
...
// Error boundary state - FIXED: Added missing interface
...
export const AuthErrorBoundary: React.FC<{ children: ReactNode }> = ({ children }) => (
  <ErrorBoundary
    level="page"
    onError={(error: Error) => { // FIXED: Added explicit type
      ErrorLogger.getInstance().logAuthError(`Auth component error: ${error.message}`);
    }}
    fallback={
      <div className="min-h-96 bg-card rounded-lg shadow p-8 text-center border border-border">
        <div className="text-4xl mb-4">🔐</div>
